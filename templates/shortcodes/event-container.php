<div data-id="<?php echo $item->id ?>" data-event-id="<?php echo $item->event_id ?>" data-start="<?php echo empty($startIndex) ? $item->start_index : $startIndex ?>" data-start-item="<?php echo $item->start_index ?>"
	...
	if ($params['title']) {
		...
		if (!$disable_url) { ?>
			<a title="<?php echo $item->post->post_title; ?>" href="<?php echo ($item->post->timetable_custom_url != "") ? $item->post->timetable_custom_url : get_permalink($item->event_id); ?>" class="event-title">
