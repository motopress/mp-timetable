	...
	<?php if (!empty($event_data)): ?>
		<?php foreach ($event_data as $data): ?>
			<tr data-id="<?php echo $data->id ?>">
				...
				<td>
					<a class="button edit-event-button" data-id="<?php echo $data->id ?>"><?php _e('Edit', 'mp-timetable') ?></a>
					<a class="button delete-event-button" data-id="<?php echo $data->id ?>"><?php _e('Delete', 'mp-timetable') ?></a>
